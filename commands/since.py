...
def command(guid, manager, irc, channel, user, show):
    ...
    if show.episode.current == show.episode.total:
        ...
        message = u"{} {} airs in {}. You are wise to be excited".format(series, ep, aired)
    elif diff.total_seconds() < 0 and alias == "jdp":
        message = u"{} {} hasn't aired yet, chill the fuck out bitch.".format(series, ep)
    elif diff.total_seconds() < 0:
        message = u"{} {} hasn't aired yet, settle down.".format(series, ep)
    elif diff.total_seconds() > 48 * 60 * 60:
        message = u"{} {} aired {} ago?! What the fuck are you doing? Get that shit done NOW!".format(series, ep, aired)
    elif diff.total_seconds() > 24 * 60 * 60:
        message = u"{} {} aired {} ago? That's fucking outrageous, why aren't you done yet?".format(series, ep, aired)
    elif diff.total_seconds() > 12 * 60 * 60:
        message = u"{} {} aired {} ago. You should probably stop screwing around with typesetting.".format(series, ep, aired)
    elif diff.total_seconds() > 10 * 60 * 60:
        message = u"{} {} aired {} ago. I do hope you're QCing right now.".format(series, ep, aired)
    elif diff.total_seconds() > 8 * 60 * 60:
        message = u"{} {} aired {} ago. Perfect time to release.".format(series, ep, aired)
    elif diff.total_seconds() > 6 * 60 * 60:
        message = u"{} {} aired {} ago. Doing just fine.".format(series, ep, aired)
    elif diff.total_seconds() > 4 * 60 * 60:
        message = u"{} {} aired {} ago. If you're still waiting for an encode, I pity you.".format(series, ep, aired)
    else:
        message = u"{} {} aired {} ago. Nothing to worry about yet.".format(series, ep, aired)
